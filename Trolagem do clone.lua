...
local function raycastChao(pos, char)
	...
	local melhorHit = nil	local melhorDist = math.huge
...
-- ==============================local function pararTodasAnimacoes()
	for nome, track in pairs(animTracks) do
		if track and track.IsPlaying then			track:Stop(0.2)
		end
	end
end
...
local function tocarAnimacao(nome, velocidade)
	...
	if track.IsPlaying then		track:AdjustSpeed(velocidade)
		return
	end
...
local function criarControlesMobile()
	...
	controls.BackgroundTransparency = 1	controls.Parent = gui

	local moveX = 0
	local moveZ = 0	local wantsJump = false
	...
	local function criarSeta(nome, texto, posX, posY, parentFrame)
		...
		btn.Text = texto
		btn.TextSize = 32		btn.TextColor3 = Color3.fromRGB(200, 220, 255)
		...
		stroke.Thickness = 2
		stroke.Transparency = 0.4		return btn
	end
	...
	dpadFrame.BackgroundTransparency = 1	dpadFrame.Parent = controls
	...
	-- BOTÃO PULO	local jumpBtn = Instance.new("TextButton")
	...
	jumpBtn.Font = Enum.Font.GothamBold	jumpBtn.AutoButtonColor = false
	...
	jumpBtn.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
			wantsJump = false			jumpBtn.BackgroundColor3 = Color3.fromRGB(0, 180, 80)
			jumpBtn.BackgroundTransparency = 0.2
		end	end)
...
local function atualizarCabeca(char, surfaceNormal)
	...
	if head then
		for _, obj in ipairs(head:GetChildren()) do			if obj:IsA("Motor6D") and obj.Name == "Neck" then
				neck = obj
				break
			end
		end
	...
	if not neck then
		...
		if upperTorso then
			for _, obj in ipairs(upperTorso:GetChildren()) do
				if obj:IsA("Motor6D") and obj.Name == "Neck" then					neck = obj
					break				end
			end
		end
	end
	...
	if not neck then
		...
		if torso then
			for _, obj in ipairs(torso:GetChildren()) do				if obj:IsA("Motor6D") and obj.Name == "Neck" then
					neck = obj
					break
				end
			end
...
local function ligar()
	...
	end	-- Para todas as animações atuais	for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
		track:Stop(0)
	end
	...
	hrp.Anchored = false	local bodyVel = Instance.new("BodyVelocity")
	bodyVel.MaxForce = Vector3.new(4e4, 4e4, 4e4)
	bodyVel.Velocity = Vector3.zero
	bodyVel.Parent = hrp	local bodyGyro = Instance.new("BodyGyro")
	...
	loop = RunService.Heartbeat:Connect(function(dt)
		...
		-- Direção de movimento já projetada na superfície (usado para andar e orientação)
		local moveDir = forward * mz + right * mx
		local isMovingOnSurface = moveDir.Magnitude > 0.1   -- <--- **CHAVE**: movimento relativo à superfície

		if isMovingOnSurface then
			-- Mantemos a direção de frente para a animação de andar
		...
		if isMovingOnSurface then
			lateralVel = moveDir.Unit * WALK_SPEED		end
		...
		if hit and dist < 5 then
			...
			if dist < STICK_DIST and verticalVelocity <= 0 then
				isGrounded = true
				verticalVelocity = 0
				jumpingFromSurface = false				local stickForce = (hit.Position + hit.Normal * STICK_DIST - hrp.Position) * 10
			...
			if jumpingFromSurface then
				verticalVelocity = verticalVelocity - GRAVITY * dt				bodyVel.Velocity = lateralVel + jumpSurfaceNormal * verticalVelocity

				if verticalVelocity < -JUMP_POWER * 2 then
					jumpingFromSurface = false				end
			else
				verticalVelocity = verticalVelocity - GRAVITY * dt
		...
		-- ============================
		-- PULO		-- ============================
		if wantsJump and isGrounded and canJump then			if isFloor then
				...
				verticalVelocity = JUMP_POWER
				isGrounded = false				jumpingFromSurface = true
			end
		...
		elseif isMovingOnSurface then          -- <--- **Usa movimento na superfície, não apenas isGrounded**
			newAnim = "walk"
		else
			newAnim = "idle"
		end
		...
		if newAnim ~= currentAnim then
			currentAnim = newAnim			tocarAnimacao(newAnim, 1)
		end

		-- Ajusta velocidade da animação de andar (mesmo em paredes/tetos)
		...
		-- ============================
		-- ORIENTAÇÃO DO CORPO (CORRIGIDO)
		-- ============================
		-- Usamos a mesma direção de movimento projetada na superfície para girar o tronco.
		local lookVec = moveDir
		if lookVec.Magnitude > 0.01 then
			lookVec = lookVec.Unit		else
			-- Quando parado, mantemos a última frente (projetada na superfície)
			lookVec = currentForward
		end
		...
		if rightVec.Magnitude > 0.01 then
			...
			rightVec = Vector3.new(1, 0, 0) -- fallback
		end
		lookVec = upVec:Cross(rightVec).Unit   -- re‑ortogonaliza
...
local function desligar()
	...
	-- Para animações	pararTodasAnimacoes()
	animTracks = {}
	...
	if char then
		...
		if hrp then
			for _, obj in ipairs(hrp:GetChildren()) do
				if obj:IsA("BodyVelocity") or obj:IsA("BodyGyro") then					obj:Destroy()
				end
			end
...
local function criarGUI()
	...
	status.Parent = frame	local btn = Instance.new("TextButton")
